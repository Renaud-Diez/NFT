...
class PHPYiiDBDataWrapper extends ArrayDBDataWrapper{
	public function select($sql){
		...
		if(is_array($filters) && count($filters) > 0){
			$criteria = $this->setFilters($filters);
		}
		
		$res = $this->connection->findAll($criteria);
	...
	protected function setFilters($filters)
	{
		...
		for ($i=0; $i < sizeof($filters); $i++)
			$criteria->compare($filters[$i]['name'], $filters[$i]['value'], false, $filters[$i]['operation']);//$arrFilters[$filters[$i]["name"]] = $filters[$i]["value"];
