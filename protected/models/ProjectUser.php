...
class ProjectUser extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('project_id, user_id, visibility', 'numerical', 'integerOnly'=>true),
			array('role', 'length', 'max'=>64),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('project_id, user_id, role, visibility', 'safe', 'on'=>'search'),
		);
	}
	...
	{
		return array(
				'ProjectUserBehavior'=>array(
				'class'=>'application.components.behaviors.ProjectUserBehavior'
				),
		);
	}
}